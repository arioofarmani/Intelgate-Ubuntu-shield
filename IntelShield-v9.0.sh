#!/usr/bin/env bash
#==============================================================================
#  IntelShield v9.0  —  System Immutability · Network Filtering · SIEM Export
...
_lock_path(){
  ...
  # Check if already immutable (avoids "Operation not supported" errors)
  if ! lsattr "$path" 2>/dev/null | grep -q '^[[:space:]]*i[- ]'; then
...
count_immutable(){
  ...
  for path in "${IMMUTABLE_PATHS[@]}"; do
    [[ -d "$path" ]] || continue
    count=$(( count + $(find "$path" -exec sh -c 'lsattr "$1" 2>/dev/null | grep -q "^[[:space:]]*i" && echo 1 || echo 0' _ {} \; 2>/dev/null | grep -c 1) ))
  done
  for path in "${IMMUTABLE_FILES[@]}"; do
    [[ -f "$path" ]] || continue
    lsattr "$path" 2>/dev/null | grep -q "^[[:space:]]*i" && count=$((count + 1))
  done
  printf '%d' "$count"
}
...
NFTEOF
  ...
  case "$mode" in
    fw)
      # Firewall mode: deterministic pipeline, default-drop
      ...
      cat >>"$ruleset" <<NFTEOF
        # Suricata FW mode: NFQUEUE with fail-open (flags bypass)
        # Default policy is DROP — only explicitly accepted packets survive
        meta l4proto { tcp, udp } queue num 0-${qmax} flags bypass,fanout
NFTEOF
      ;;
    ips)
      # IPS mode: traditional inline NFQUEUE
      local ncpu qmax i qargs=""
      ncpu=$(nproc 2>/dev/null || echo 1); qmax=$((ncpu-1)); (( qmax>3 )) && qmax=3; (( qmax<0 )) && qmax=0
      for ((i=0;i<=qmax;i++)); do qargs+=" -q $i"; done
      cat >>"$ruleset" <<NFTEOF
        # Suricata IPS: NFQUEUE with fail-open
        meta l4proto { tcp, udp } queue num 0-${qmax} flags bypass,fanout
NFTEOF
      ;;
    *)
...
nft_sync(){
  local mode="off"
  local suri_active suri_fw crowdsec_active

  suri_active="$(systemctl is-active suricata 2>/dev/null || echo inactive)"
  suri_fw="$(cat "$SURICATA_FW_MODE_FILE" 2>/dev/null || echo off)"
  crowdsec_active="$(systemctl is-active crowdsec-firewall-bouncer 2>/dev/null || echo inactive)"
  ...
  deploy_nft_pipeline "$CROWDSEC_PRIORITY" "$SURICATA_FW_PRIORITY" "${SSH_PORT:-22}" "$mode"
}
